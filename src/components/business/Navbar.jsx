...
const Navbar = ({ onSearch, theme = 'light' }) => {
    ...
    const baseClasses = {
        ...
        dropdownMenu: `absolute z-10 right-0 mt-4 w-64 py-3 rounded-2xl shadow-2xl border backdrop-blur-sm ${
            theme === 'light' ? 'bg-white/95 border-gray-100' : 'bg-gray-900/95 border-gray-800'
        }`,
        dropdownItem: `w-full px-4 py-2 text-left flex items-center space-x-3 transition-all duration-200 ${
            theme === 'light' ? 'text-gray-700 hover:bg-gray-50' : 'text-gray-100 hover:bg-gray-800'
        }`
    };
                    ...
                    <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                        <svg 
                            className={`w-5 h-5 transition-colors duration-200 ${
                                theme === 'light' ? 'text-gray-400 group-hover:text-gray-600' : 'text-gray-500 group-hover:text-gray-400'
                            }`}
                            fill="none" 
                            viewBox="0 0 24 24" 
                            stroke="currentColor"
            ...
            <div className="flex items-center space-x-3">
                ...
                <div className="relative" ref={dropdownRef}>
                    ...
                    >
                        ...
                        <svg 
                            className={`w-4 h-4 transition-transform duration-200 ${showDropdown ? 'transform rotate-180' : ''} ${
                    ...
                    {showDropdown && (
                        <div className={baseClasses.dropdownMenu}>
                            <div className="px-5 pb-3 mb-2 border-b border-gray-100">
                                ...
                                <p className="font-semibold mt-0.5 text-sm">john.doe@example.com</p>
                            </div>
                            <button onClick={handleProfileClick} className={baseClasses.dropdownItem}>
                                <svg 
                                    className={`w-5 h-5 ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}
                                    fill="none" 
                                    viewBox="0 0 24 24" 
                                    stroke="currentColor"
                                >
                            ...
                            <button onClick={handleLogout} className={baseClasses.dropdownItem}>
                                <svg 
                                    className={`w-5 h-5 ${theme === 'light' ? 'text-gray-500' : 'text-gray-400'}`}
                                    fill="none" 
                                    viewBox="0 0 24 24" 
