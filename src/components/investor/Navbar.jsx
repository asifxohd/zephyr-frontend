...
import { handleListNotifications } from '@/src/services/api/notification';
							...
							<div className="flex items-center space-x-6">
								...
								<div className="relative group">
                                   ...
                                   rounded-full flex items-center justify-center ring-2 ring-white">
											<span className="text-[10px] text-white font-bold">{notifications ? notifications.length : "0"}</span>
										</span>
									</div>
										...
										<div className="p-4">
											...
											<div className="space-y-3">
												{notifications.map((notification) => (
													<div key={notification.id} className="flex items-start space-x-3 p-2 hover:bg-blue-50 rounded-lg cursor-pointer">
														<div className="bg-blue-100 p-2 rounded-lg">
															<Bell className="h-4 w-4 text-blue-500" />
														</div>
														<div>
															<div className="text-xs text-gray-500">{notification.message}</div>
															<div className="text-xs text-gray-400 mt-1">{notification.created_at ?formatTimeAgo(notification.created_at): formatTimeAgo(notification.created_at)}</div>
														</div>
													</div>
												))}
											</div>
								...
								<div className="relative group">
                                  ...
                                  transform transition-all duration-200">
											<div className="p-4 border-b border-gray-100">
												<div className="text-sm font-semibold text-gray-900">{userInformations.name ? userInformations.name: "Not given"}</div>
...
function formatTimeAgo(isoString) {
	...
	// Handle if the input is just a time (e.g., "09:29:09")
	if (isNaN(date.getTime())) {
	  // If it's a time string, add today's date for comparison
	...
	if (diffInSeconds < 60) {
	  ...
	  return date.toLocaleDateString(); // Return the full date if more than a week ago
	}
  }
